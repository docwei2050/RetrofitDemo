...
public final class Retrofit {
  ...
  private void validateServiceInterface(Class<?> service) {
    ...
    while (!check.isEmpty()) {
      ...
      //类型上面加上泛型是不可以的candidate.getTypeParameters()---就是泛型参数
  ...
  //一旦缓存过，下次就直接复用之前的，这个有点厉害哈，但是没有地方清除请求
...
//    Objects.requireNonNull(annotations, "annotations == null");
  ...
  public CallAdapter<?, ?> nextCallAdapter(
    ...
    for (int i = start, count = callAdapterFactories.size(); i < count; i++) {
      //如果是RxJava2CallAdapterFactory的话，只满足返回值是Observable Maybe Signle Flowable才行，不然就返回
...
//    Objects.requireNonNull(annotations, "annotations == null");
  ...
  public <T> Converter<ResponseBody, T> nextResponseBodyConverter(
    ...
    for (int i = start, count = converterFactories.size(); i < count; i++) {
      ...
      if (converter != null) { //先走BuiltInConvertes，然后ScalarsConverterFactory、GsonConverterFactory
        //noinspection unchecked
